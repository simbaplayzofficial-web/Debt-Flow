...
export default function Profile() {
  ...
  const peopleIOwe = debts.filter(d => {
    if (d.user1Id === currentUser?.id) return d.netBalance < 0;
    if (d.user2Id === currentUser?.id) return d.netBalance > 0;
    return false;
  }).map(d => {
  ...
  const currentWarningDescription = () => {
    ...
    if (level === 2) return `Must complete ${currentUser.communityServicesNeeded} community services. Cannot gain new debts.`;
    if (level === 3) return `Restricted until ${new Date(currentUser.restrictedUntil || '').toLocaleDateString()}. Cannot request work or run for admin.`;
    ...
    <div className="space-y-8">
      ...
      <div className="grid sm:grid-cols-3 gap-6">
        <div className="bg-neutral-900 border border-neutral-800 p-8 rounded-3xl relative overflow-hidden group">
          ...
          <p className="text-neutral-500 text-[10px] font-bold uppercase tracking-widest mb-4">Investments</p>
          <p className="text-4xl font-black italic mb-2 text-green-500">{totalOwedToMe}</p>
        ...
        <div className="bg-neutral-900 border border-neutral-800 p-8 rounded-3xl relative overflow-hidden group">
          ...
          <p className="text-neutral-500 text-[10px] font-bold uppercase tracking-widest mb-4">Liabilities</p>
          <p className="text-4xl font-black italic mb-2 text-red-500">{totalOwedByMe}</p>
          <p className="text-neutral-400 text-sm">Debt you owe others</p>
        </div>

        <div className="bg-neutral-900 border border-neutral-800 p-8 rounded-3xl relative overflow-hidden group">
           <div className="absolute top-0 right-0 p-4 opacity-10">
            <ShieldAlert size={100} className={getIntegrityColor(currentUser?.integrityLevel || 0)} />
          </div>
          <p className="text-neutral-500 text-[10px] font-bold uppercase tracking-widest mb-4">Integrity Level</p>
          <div className="flex items-end gap-3 mb-2">
            <p className={`text-4xl font-black italic ${getIntegrityColor(currentUser?.integrityLevel || 0)}`}>
              LVL {currentUser?.integrityLevel ?? 0}
            </p>
            <p className="text-neutral-500 text-[10px] font-bold uppercase mb-1">{currentUser?.integrityScore}% Score</p>
          </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
            ...
            <div className="p-8">
               {activeSettingsTab === 'info' ? (
                 <form onSubmit={handleUsernameUpdate} className="space-y-6">
                    <div>
                      <label className="block text-[10px] font-bold text-neutral-500 uppercase tracking-widest mb-2">Display Username</label>
                      <input 
                        type="text" 
                        value={newUsername}
                      ...
                      />
                      <p className="text-[9px] text-neutral-600 mt-2 italic">Requires uniqueness protocol check before validation.</p>
                    </div>
                    <button 
                      type="submit"
                      disabled={isUpdatingProfile || newUsername === currentUser?.username}
                 ...
                 <form onSubmit={handlePasswordUpdate} className="space-y-6">
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="relative">
                          <Key className="absolute left-4 top-1/2 -translate-y-1/2 text-neutral-700" size={14} />
                          <input 
                            type="password" 
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-neutral-500 uppercase tracking-widest mb-2">New Password Secret</label>
                        <div className="relative">
                          <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-neutral-700" size={14} />
                          <input 
                            type="password" 
                            value={newPassword}
                    ...
                    <button 
                      type="submit"
                      disabled={isUpdatingProfile || !currentPassword || newPassword.length < 6}
                      className="w-full bg-neutral-800 hover:bg-neutral-700 disabled:opacity-50 text-neutral-200 text-[10px] font-black py-3 rounded-xl uppercase tracking-widest transition-all border border-neutral-700"
                    >
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-neutral-500">
                             ...
                             <span className={`ml-2 uppercase font-black px-1 rounded text-[8px] ${
                                ...
                                tx.status === 'rejected' ? 'bg-red-500/10 text-red-500' : 'bg-green-500/10 text-green-500'
                             }`}>[{tx.status}]</span>
                          </p>
                        </div>
                      </div>
        ...
        <div className="space-y-8">
            ...
            <section>
               ...
               <div className="space-y-6">
                 ...
                 {peopleWhoOweMe.length > 0 && (
                   <div>
                     <p className="text-[10px] font-black text-green-500 uppercase tracking-widest mb-3">People who owe me</p>
                     <div className="space-y-2">
                       {peopleWhoOweMe.map(item => (
                         <div key={item.user?.id} className="bg-neutral-900 border border-neutral-800 p-4 rounded-xl flex items-center justify-between">
                            <p className="text-xs font-bold text-neutral-200">@{item.user?.username}</p>
                            <p className="text-xs font-black text-green-500">{item.balance} DB</p>
                         </div>
                       ))}
                     </div>
                   </div>
                 ...
                 {peopleIOwe.length > 0 && (
                   <div>
                     <p className="text-[10px] font-black text-red-500 uppercase tracking-widest mb-3">People I owe</p>
                     <div className="space-y-2">
                       {peopleIOwe.map(item => (
                         <div key={item.user?.id} className="bg-neutral-900 border border-neutral-800 p-4 rounded-xl flex items-center justify-between">
                            <p className="text-xs font-bold text-neutral-200">@{item.user?.username}</p>
                            <p className="text-xs font-black text-red-500">{item.balance} DB</p>
                         </div>
                       ))}
                     </div>
                   </div>
                 ...
                 {peopleWhoOweMe.length === 0 && peopleIOwe.length === 0 && (
                   <div className="bg-neutral-900/50 border border-neutral-800 border-dashed p-6 rounded-2xl text-center">
                     <p className="text-neutral-500 text-[10px] font-bold uppercase tracking-widest">Balanced Ledger</p>
                   </div>
                 )}
               </div>
            </section>
           ...
           <section>
              ...
              <div className="space-y-4">
                {userWarnings.sort((a,b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()).map(warn => (
                  <div key={warn.id} className="bg-neutral-900 border border-red-500/10 p-5 rounded-2xl border-l-4 border-l-red-500">
                    <div className="flex justify-between items-start mb-2">
                       <p className="text-xs font-black text-red-500 uppercase tracking-widest">Level {warn.level} Violation</p>
                       <p className="text-[10px] text-neutral-500 font-mono">{new Date(warn.timestamp).toLocaleDateString()}</p>
                    </div>
                    <p className="text-sm text-neutral-200 leading-relaxed font-medium">"{warn.reason}"</p>
                    <div className="mt-3 pt-3 border-t border-neutral-800 flex items-center justify-between">
                       <p className="text-[10px] text-neutral-500 uppercase tracking-widest">
                         Issued By: {users.find(u => u.id === warn.issuedBy)?.username || 'System Admin'}
                       </p>
                       {warn.level >= 3 && <span className="text-[8px] bg-red-500/20 text-red-400 px-1.5 py-0.5 rounded font-black uppercase">RESTRICTED</span>}
                    </div>
                  </div>
                ))}
                {userWarnings.length === 0 && (
                  <div className="bg-neutral-900/50 border border-neutral-800 border-dashed p-10 rounded-2xl text-center">
                    <ShieldCheck size={32} className="mx-auto text-green-500/30 mb-3" />
                    <p className="text-neutral-500 text-xs font-bold uppercase tracking-widest">No violations on record</p>
                  </div>
                )}
              </div>
           </section>
                ...
                <div className="grid gap-3">
                   ...
                   <button 
                      onClick={() => {
                        ...
                        if (user) {
                          const level = parseInt(prompt("Enter warning level (1-5):") || "0");
               ...
               <form onSubmit={handleAddTransaction} className="space-y-4">
                 ...
                 <div>
                    ...
                    >
                       ...
                       {users.filter(u => u.id !== currentUser?.id && !u.isPermanentlyRemoved).map(u => (
